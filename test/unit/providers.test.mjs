...
import { PROVIDERS, describeError, providerFor, trimSlash } from "../../extension/bg/providers.js";
...
describe("trimSlash / providerFor", () => {
  ...
  it("fällt bei unbekanntem Provider auf Lokal zurück", () => {
    assert.equal(providerFor({ provider: "huggingface" }), PROVIDERS.huggingface);
    assert.equal(providerFor({ provider: "weg" }), PROVIDERS.local);
  });
});
...
describe("Lokal / Eigener Server (Vertrag POST {texts, model?} -> {scores})", () => {
  it("Lokal: /v1/score, Modell im Body, kein Authorization-Header", async () => {
    ...
    assert.deepEqual(await PROVIDERS.local.score(["a", "b"], local), [0.1, 0.9]);
  ...
  it("Eigener Server: URL unverändert, Bearer-Key, ohne Modell kein model-Feld", async () => {
    ...
    await PROVIDERS.custom.score(["a"], { ...custom, customApiKey: "geheim" });
  ...
  it("Nicht-Zahlen werden null (Absatz bleibt unbewertet), der Rest bleibt", async () => {
    ...
    assert.deepEqual(await PROVIDERS.custom.score(["a", "b", "c"], custom), [0.2, null, null]);
  });

  it("lehnt Antworten ohne passendes scores-Array ab", async () => {
    for (const body of [{ scores: [0.1] }, { score: [0.1, 0.2] }, { scores: "0.1,0.2" }]) {
      mockFetch({ body });
      await assert.rejects(PROVIDERS.custom.score(["a", "b"], custom), /passendes 'scores'-Array/);
    }
  });

  it("Eigener Server ohne URL: Fehler statt Anfrage", async () => {
    mockFetch();
    await assert.rejects(async () => PROVIDERS.custom.score(["a"], { ...custom, customUrl: "" }), /Keine Server-URL/);
  ...
  it("HTTP-Fehler mit Detail aus dem Body (FastAPI, OpenAI-Stil, Objekt, kein JSON)", async () => {
    ...
    for (const [resp, message] of cases) {
      mockFetch(resp);
      await assert.rejects(PROVIDERS.local.score(["a"], local), { message });
    }
  });
});
...
describe("Hugging Face", () => {
  it("Anfrage: Router-URL mit Modell, Token als Bearer, Texte als inputs", async () => {
    ...
    assert.deepEqual(await PROVIDERS.huggingface.score(["a"], hf), [0.8]);
  ...
  it("Antwortformen: pro Text eine Liste, Einzeltext flach, Top-1 flach", async () => {
    ...
    assert.deepEqual(await PROVIDERS.huggingface.score(["a", "b"], hf), [0.9, 0.3]);

    mockFetch({ body: [{ label: "Human", score: 0.6 }, { label: "ChatGPT", score: 0.4 }] });
    assert.deepEqual(await PROVIDERS.huggingface.score(["a"], hf), [0.4]);

    mockFetch({ body: [{ label: "LABEL_1", score: 0.95 }, { label: "LABEL_0", score: 0.75 }] });
    assert.deepEqual(await PROVIDERS.huggingface.score(["a", "b"], hf), [0.95, 0.25]);
  });
  ...
  it("automatische Label-Erkennung", async () => {
    ...
    for (const [label, score] of labels) {
      ...
      assert.deepEqual(await PROVIDERS.huggingface.score(["a"], hf), [score], label);
    }
    // Top-1 = Mensch -> Gegenwahrscheinlichkeit; ohne Top-1 lässt sich nichts ableiten
    mockFetch({ body: [[{ label: "human-written", score: 0.75 }]] });
    assert.deepEqual(await PROVIDERS.huggingface.score(["a"], hf), [0.25]);
  });
  ...
  it("manuelles KI-Label (Groß-/Kleinschreibung egal) überstimmt die Automatik", async () => {
    ...
    assert.deepEqual(await PROVIDERS.huggingface.score(["a"], { ...hf, hfAiLabel: "synthetic" }), [0.8]);
  });
  ...
  it("einzelne Texte ohne KI-Label werden null, alle ohne -> Fehler mit den gesehenen Labels", async () => {
    ...
    assert.deepEqual(await PROVIDERS.huggingface.score(["a", "b"], hf), [0.9, null]);

    mockFetch({ body: [[{ label: "POSITIVE", score: 0.9 }, { label: "NEGATIVE", score: 0.1 }]] });
    await assert.rejects(PROVIDERS.huggingface.score(["a"], hf), /Modell liefert: POSITIVE, NEGATIVE/);
  });

  it("Fehler: kein Modell, keine Liste, falsche Anzahl", async () => {
    mockFetch();
    await assert.rejects(PROVIDERS.huggingface.score(["a"], { ...hf, hfModel: "" }), /Kein Hugging-Face-Modell/);
    assert.equal(calls.length, 0);

    mockFetch({ body: { error: "Model is loading" } });
    await assert.rejects(PROVIDERS.huggingface.score(["a"], hf), /Unerwartete Antwort/);

    mockFetch({ body: [[{ label: "AI", score: 0.9 }]] });
    await assert.rejects(PROVIDERS.huggingface.score(["a", "b"], hf), /Anzahl Ergebnisse/);

    mockFetch({ status: 503, body: { error: "Model is loading" } });
    await assert.rejects(PROVIDERS.huggingface.score(["a"], hf), { message: "HTTP 503: Model is loading" });
  });
});
...
describe("Im Browser", () => {
  it("fragt das Offscreen-Dokument (legt es bei Bedarf an) und reicht die Scores durch", async () => {
    ...
    assert.deepEqual(await PROVIDERS.browser.score(["a"], { provider: "browser", browserModel: "desklib" }), [0.3]);
  ...
  it("Fehler aus dem Offscreen-Dokument kommen als Fehler an", async () => {
    ...
    await assert.rejects(PROVIDERS.browser.score(["a"], { browserModel: "tmr" }), /Modell nicht heruntergeladen/);
    globalThis.chrome.runtime.sendMessage = async () => undefined;
    await assert.rejects(PROVIDERS.browser.score(["a"], { browserModel: "tmr" }), /antwortet nicht/);
  });
});
